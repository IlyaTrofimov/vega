...
class MFKD1(SearchAlgorithm):
    ...
    def _get_best_arc(self):
        ...
        for i in range(len(self.points)):
            ...
            y_train.append(self.acc_list[i] / 100)

        gpr = GPR(kernel = RBF(1.0))
        gpr.fit(X_train, y_train)

        preds, std = gpr.predict(self.X, return_std = True)
        best_idx = np.argmax([x[0]-0.5*x[1] for x in zip(preds, std)])
    ...
    def update(self, worker_path):
        ...
        if self.is_completed:
            self.best_model_idx = self._get_best_arc()
